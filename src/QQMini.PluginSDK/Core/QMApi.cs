...
namespace QQMini.PluginSDK.Core
{
	...
	public sealed class QMApi
	{
		...
		public Version GetFrameVersion ()
		{
			...
			if (Version.TryParse (versionPtr.ToString (Global.DefaultEncoding), out Version version))
			{
				return version;
			}

			throw new FormatException ("无法解析指定的版本号");
		}
		/// <summary>
		/// 获取框架内部时间戳
		/// </summary>
		...
		{
			IntPtr dirPtr = QQMiniApi.QMApi_GetPluginDataDirectory (this._authCode);
			return dirPtr.ToString (Global.DefaultEncoding);
		}
		/// <summary>
		/// 获取指定QQ的好友列表
		/// </summary>
		...
		/// <returns>如果获取成功返回指定QQ的好友列表集合</returns>
		public ReadOnlyCollection<object> GetFriendList (long responseQQ, bool useCache = true)
		{
			if (QQMiniApi.QMApi_GetFriendList (this._authCode, responseQQ, useCache, out IntPtr szFriendList) == 0)
			{
				string base64Data = szFriendList.ToString (Global.DefaultEncoding);
			...
			throw new InvalidDataException ($"获取“{responseQQ}”的好友列表失败");
		}
		/// <summary>
		/// 获取指定QQ的好友信息,
		/// </summary>
		...
		/// <param name="xmlSubType">基本结构(0), 歌曲结构(2), 其他值不明. 参考 #XML_. 如果是普通消息和JSON消息, 本参数为空</param>
		/// <returns>成功返回 0, 失败返回负值</returns>
		public int SendMessage (long responseQQ, int sendType, long destTarget, long destQQ, string message, int xmlSubType)
		{
			GCHandle messageHandle = message.GetStringGCHandle (Global.DefaultEncoding);
			try
			{
				return QQMiniApi.QMApi_SendMessage (this._authCode, responseQQ, sendType, destTarget, destQQ, messageHandle.AddrOfPinnedObject (), xmlSubType);
			}
			finally
			{
